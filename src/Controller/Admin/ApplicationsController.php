...
class ApplicationsController extends AppController
{
    ...
    public function pdfList()
    {
        ...
        $this->paginate = [
            'contain' => ['Users', 'Faculties', 'Programs', 'Semesters', 'Branches'],
            'maxLimit' => 10,
        ];
    ...
    public function index()
    {
        ...
        $programs = $this->Applications->Programs->find('list', ['keyField' => 'id', 'valueField' => function ($program) {
            return $program->code . ' : ' . $program->name;
        }, 'limit' => 200])->all();
        //$semesters = $this->Applications->Semesters->find('list', limit: 200)->all();
        $semesters = $this->Applications->Semesters->find('list', ['keyField' => 'id', 'valueField' => function ($semester) {
            return $semester->code . ' : ' . $semester->session;
        }, 'limit' => 200])->all();
        $branches = $this->Applications->Branches->find('list', limit: 200)->all();
        $this->set(compact('applications', 'users', 'faculties', 'programs', 'semesters', 'branches', 'monthArray', 'countArray'));
    ...
    public function view($id = null)
    {
        ...
        $application = $this->Applications->get($id, contain: ['Users', 'Faculties', 'Programs', 'Semesters', 'Branches']);
        ...
        $semesters = $this->Applications->Semesters->find('list', limit: 200)->all();
        $branches = $this->Applications->Branches->find('list', limit: 200)->all();
        $this->set(compact('application', 'users', 'faculties', 'programs', 'semesters', 'branches'));
    ...
    public function pdf($id = null)
    {
        ...
        $this->viewBuilder()->setOption(
            ...
            [
                ...
                'filename' => 'Application_' . $id . '.pdf' //// This can be omitted if you want file name based on URL.
            ]
        );
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            $application = $this->Applications->patchEntity($application, $this->request->getData());
            $application->user_id = $this->Authentication->getIdentity('id')->getIdentifier('id'); //capture auth id
            if ($this->Applications->save($application)) {
                $this->Flash->success(__('The application has been saved.'));
